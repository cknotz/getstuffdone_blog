...
Merging two datasets can seem a bit daunting to beginners, but it is actually not that difficult. The `dplyr` package has functions for so-called "mutating joins" (see [https://dplyr.tidyverse.org/reference/mutate-joins.html](https://dplyr.tidyverse.org/reference/mutate-joins.html){target="_blank"}), which in essence simply merge two datasets along one (or potentially more) "identifier" variables.^[If you want a more detailed explanation of how to merge two datasets, you can read [this post](https://cknotz.github.io/getstuffdone_blog/posts/merging/), which also contains links to other online resources, or Chapter 11.2 in Urdinez & Cruz [-@Urdinez2020].] Identifier variables are variables that uniquely identify a given observation in both datasets so that `R` knows exactly which row in dataset 1 corresponds to which row in dataset 2. As long as one has one (or more) variables that can directly match observations between datasets -- which in our case is the two-number ISCO-88 score -- one can use these functions to merge two datasets.
