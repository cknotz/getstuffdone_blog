...
lightbox: true
citation:
  url: https://cknotz.github.io/getstuffdone_blog/posts/conflict/
lang: en
categories:
  - Macro
  - UCDP
...
This graph is a bit messy and difficult to read, but it makes clear that most countries are covered for the entire period. Let's have a closer look at those that have less than full coverage:
```{r}
ged %>% 
  group_by(country_cy) %>% 
  summarise(start_year = min(year_cy, na.rm = T),
