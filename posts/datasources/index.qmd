...
Finding relevant data is one of the major hurdles which many students face during term paper or thesis projects --- which is a bit puzzling given that there is a *huge* and growing amount of data about many political, social, or economic aspects freely available on the open internet. 

This post provides a (definitely not exhaustive) list of datasets and data repositories that can be relevant for research projects in the social and political sciences. It is definitely not the first such list (see below),  but it complements other lists with a stronger focus on datasets from and relevant for research on welfare states.

## Other (better?) overviews over available data

Erik Gahner's excellent [*Dataset of Political Datasets*](https://github.com/erikgahner/PolData){target="_blank"} provides a great overview over datasets that are relevant to political science, and there are also a number of data repositories and archives that contain even more (and partly older) data that might be relevant for some project, for example
...
### Technology & technological change
 ...
 - OECD [Risks that Matter](https://www.oecd.org/social/risks-that-matter.htm) surveys: Cross-country comparative survey data, incl. questions on perceived vulnerability to technological change [see e.g., @Busemeyeretal2023]
